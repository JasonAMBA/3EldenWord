...
exports.login = async (req, res) => {
  try {
    ...
    // Eviter l'accumulation de tokens de rafraichissement
...
exports.refreshToken = async (req, res) => {
  try {
    ...
    try {
      ...
      // Génération d'un nouveau refreshToken
...
exports.logout = async (req, res) => {
  try {
    ...
    // Vérification de l'existence du token en base
    const refreshTokenQuery = "SELECT * FROM tokens WHERE refresh_token = ?";
    const token = await query(refreshTokenQuery, [refreshToken]);
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message : "Suppression du compte réussi !" });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: "Erreur lors de la suppression du compte de l'utilisateur !" });
  }
}