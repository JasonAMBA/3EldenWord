...
exports.startLevel = async (req, res) => {
  try {
    const userId = req.user.id; // Récupéré depuis le token d'accès
    ...
    const existingProgress = await query("SELECT * FROM progress WHERE user_id = ? AND boss_id = ?", [userId, bossId]);

    // Si la progression existe déjà et que is_completed est egal à true
    if (existingProgress.length > 0 && existingProgress[0].is_completed === 1) {
      return res.status(200).json({message: "Niveau déjà terminé !"});
    }

    // Si la progression existe déjà et qu'elle est incomplète
    if (existingProgress.length > 0) {
      return res.status(200).json({message : "Niveau déjà en cours"})
    }
    ...
    if (level > 1) {
      const previousBoss = await query("Select id FROM bosses WHERE region_id = ? AND level = ?", [region_id, level - 1]);
      ...
      const previousCompleted = await query("SELECT * FROM progress WHERE user_id = ? AND boss_id = ? AND is_completed = 1",[userId, previousBossId]);
    ...
    await query("INSERT INTO progress (user_id, boss_id, is_completed) VALUES (?, ?, ?)",[userId, bossId, false]);

    // Créer l'entrée pour les tentatives (avec 0 par défaut)
    await query("INSERT INTO guesses (user_id, boss_id, attempt_number) VALUES (?, ?, ?)",[userId, bossId, 0]);
...
exports.makeGuess = async (req, res) => {
  try {
    ...
    const query = util.promisify(db.query).bind(db);
    const progress = await query("SELECT * FROM progress WHERE user_id = ? AND boss_id = ? AND is_completed = 0", [userId, bossId]);
    ...
    const attempts = await query("SELECT * FROM guesses WHERE user_id = ? AND boss_id = ?", [userId, bossId]);

    if (attempts.length === 0) {
      return res.status(404).json({message: "Les tentatives n'ont pas été initialisés !"});
    }
    ...
    // Vérifier si le jouer a dépassé 5 tentatives
    ...
    if (normalize(guess) === normalize(correctAnswer)) {
      ...
      await query("UPDATE guesses SET attempt_number = ?, last_attempt = CURRENT_TIMESTAMP WHERE user_id = ? AND boss_id = ?", [newAttemptCount, userId, bossId]);
